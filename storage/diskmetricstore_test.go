...
func TestAddDeletePersistRestore(t *testing.T) {
	...
	dms := NewDiskMetricStore(fileName, 100*time.Millisecond, nil, logger, 60*time.Second, 1)
...
func TestNoPersistence(t *testing.T) {
	dms := NewDiskMetricStore("", 100*time.Millisecond, nil, logger, 60*time.Second, 1)
...
func TestHelpStringFix(t *testing.T) {
	dms := NewDiskMetricStore("", 100*time.Millisecond, prometheus.DefaultGatherer, logger, 60*time.Second, 1)
